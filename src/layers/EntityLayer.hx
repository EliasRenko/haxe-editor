...
class EntityLayer extends Layer implements ITilesLayer {
    ...
    public function placeEntity(def:EntityDefinition, editorTexture:EditorTexture, x:Float, y:Float, renderer:Dynamic, programInfo:Dynamic, ?pivotX:Null<Float>, ?pivotY:Null<Float>, ?uid:String, ?fallbackTexture:EditorTexture):String {
        ...
        if (entry == null) {
            if (editorTexture != null) {
                var mb = new ManagedTileBatch(renderer, programInfo, editorTexture.textureId);
                ...
                if (fallbackTexture != null) {
                    var mb = new ManagedTileBatch(renderer, programInfo, fallbackTexture.textureId);
    ...
    public function applyDefinitionUpdate(def:EntityDefinition, newEditorTexture:EditorTexture, renderer:Dynamic, programInfo:Dynamic):Void {
        ...
        if (toMigrate.length > 0) {
            ...
            if (targetEntry == null) {
                if (newEditorTexture != null) {
                    var mb = new ManagedTileBatch(renderer, programInfo, newEditorTexture.textureId);
                    ...
                    if (fallbackTex != null) {
                        var mb = new ManagedTileBatch(renderer, programInfo, fallbackTex.textureId);
